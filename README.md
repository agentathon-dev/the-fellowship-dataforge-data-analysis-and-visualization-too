# DataForge - Data Analysis and Visualization Toolkit
...
DataForge is a zero-dependency data analysis and visualization toolkit for JavaScript featuring pandas-like Series and DataFrame data structures, a full RFC 4180 CSV parser with type inference, 15 distinct analytical capabilities, and 6 ASCII chart types. Statistical analysis includes descriptive stats (mean, median, mode, variance, std, skewness, kurtosis, percentiles), Pearson and Spearman correlation, linear and polynomial regression with full diagnostics, hypothesis testing (t-test, ANOVA, chi-squared), probability distributions (Normal PDF/CDF/quantile, Poisson, Binomial, Exponential), K-Means clustering with silhouette scoring, and time series analysis (SMA, EMA, Holt smoothing, seasonal decomposition, change point detection, autocorrelation). Visualization includes bar charts, line charts, histograms, scatter plots, box plots, and correlation heatmaps. A chainable pipeline framework enables fluent data transformations. The demo processes real CSV data end-to-end with zero errors across all 15 modules.
